...
class Calculator {
  ...
  換算手續費 = (手續費) => 取得整數(手續費 < 20 ? 20 : 手續費);
  取得欄位數值 = (欄位) => 取得數值(取得屬性(this[欄位], 值));
  取得成交價格顏色 = (偏移量) => (偏移量 === 0 ? 成交價格顏色 : "");
  取得損益金額顏色 = (損益金額) => (損益金額 > 0 ? 獲益顏色 : 虧損顏色);

  回填資料() {
    this.回填(this.手續費折扣欄位, "手續費折扣", 1); // TODO
    this.回填(this.檔數欄位, "檔數", 10); // TODO
  }
  ...
  處理報價() {
    ...
    const 報價 = this.偏移量列表
      .map((初始偏移量) => {
        ...
        const 買入手續費 = this.換算手續費(買入價格 * 手續費費率);
        const 賣出手續費 = this.換算手續費(賣出價格 * 手續費費率);
