...
const 買入價格欄位 = 取得元素("買入價格");
const 賣出價格欄位 = 取得元素("賣出價格");
...
const 處理報價 = () => {
  const 買入價格 = 取得數值(取得屬性(買入價格欄位, 值));
  const 賣出價格 = 取得數值(取得屬性(賣出價格欄位, 值));
  ...
  if (!買入價格 || !賣出價格 || !交易股數 || !手續費折扣) {
    return;
  }

  const 報價 = 取得偏移量()
    .map((初始偏移量) => {
      const 成交價檔位 = 取得數值(賣出價格欄位[檔位]);
      const 偏移量 = (初始偏移量 * 千 * 成交價檔位) / 千;
      const 買進價格 = 買入價格 * 交易股數;
      const 賣出價格格 = (賣出價格 + 偏移量) * 交易股數;
      const 手續費費率 = 公定手續費費率 * 手續費折扣;
      const 買入手續費 = 取得最低手續費(取得整數(買進價格 * 手續費費率));
      const 賣出手續費 = 取得最低手續費(取得整數(賣出價格格 * 手續費費率));
      const 證券交易稅 = 取得整數(賣出價格格 * 證券交易稅稅率);
      const 支付總金額 = 買進價格 + 買入手續費;
      const 實收總金額 = 賣出價格格 - 賣出手續費 - 證券交易稅;
